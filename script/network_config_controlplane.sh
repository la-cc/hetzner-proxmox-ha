...
read -p "NUMBER_OF_NODES (e.g. 3 Nodes (1x Master, 2x Worker, only worker nodes needed) = 1 2): " NUMBER_OF_NODES

for node in ${NUMBER_OF_NODES}; do
  read -p "VSWITCH_COROSYNC: " VSWITCH_COROSYNC
  read -p "IP_ADRESS_COROYSYNC (e.g. 10.0.100.10, 10.0.200.10, etc.) " IP_ADRESS_COROYSYNC
  echo "
